...
class MSPPDataPlotter:
    ...
    def setup_ui(self):
        ...
        ttk.Button(
            plot_frame,
            text="📊 E.coli vs Yeast Fold Change",
        ...
        ttk.Button(
            plot_frame,
            text="📊 Both Organisms vs HeLa",
            command=self.plot_organisms_vs_hela,
            width=35,
        ).pack(pady=5, fill=tk.X, padx=10)
        ...
        pattern_entry = tk.Entry(
            group_frame,
            textvariable=self.group_pattern,
            width=40,
            bg=self.DARK_ACCENT,
            fg=self.DARK_FG,
        ...
        ttk.Label(
            group_frame,
            text="Default: '(E\\d+)' groups by E.coli ratio (E25, E100, etc.)",
            font=("Arial", 8),
        ).pack(anchor=tk.W)

        ttk.Button(
            group_frame,
            text="📦 Grouped Fold Change Box Plot",
            command=self.plot_grouped_fold_change,
            width=35,
    ...
    def _create_fold_change_plot(self, data, ax):
        ...
        sample_names, all_protein_fcs, protein_counts = self._calculate_fold_changes_per_sample(
            data
        )
        ...
        bp = ax.boxplot(
            ...
            meanprops=dict(
                marker="s", markerfacecolor="white", markeredgecolor="white", markersize=4
            ),
        )
        ...
        ax.set_xticklabels(
            [s.replace("report.pg_matrix_", "") for s in sorted_names],
            rotation=45,
            ha="right",
            fontsize=9,
        )
    ...
    def plot_both(self):
        """Generate all three plots simultaneously."""
        data = self.load_data()
        if data is None:
            return
        ...
        # Plot 2: E.coli vs Yeast fold change
        fig2, ax2 = plt.subplots(figsize=(12, 7))
        success = self._create_fold_change_plot(data, ax2)
        if success:
            fig2.tight_layout()
        else:
            plt.close(fig2)

        # Plot 3: Organisms vs HeLa
        self.plot_organisms_vs_hela()
    ...
    def _plot_single_organism_vs_hela(self, ax, data, organism):
        ...
        # Sort by median
        sorted_pairs = sorted(
            zip(sample_data, sample_names), key=lambda x: np.median(x[0]), reverse=True
        )
    ...
    def plot_organisms_vs_hela(self):
        ...
        plt.suptitle(
            "Spike-in Validation: Expected Ratios vs Observed",
            fontsize=15,
            fontweight="bold",
            y=0.98,
        )
    ...
    def plot_grouped_fold_change(self):
        ...
        if unmatched:
            messagebox.showwarning(
                "Warning", f"{len(unmatched)} files did not match the pattern and will be excluded."
            )
        ...
        for i, group in enumerate(sorted_groups):
            ...
            ax.text(
                ...
                (
                    ax.get_ylim()[1] * 0.95
                    if ax.get_ylim()[1] > 0
                    else max([max(d) for d in box_data]) + 0.3
                ),
                f"{len(groups[group])} proteins",
                fontsize=9,
                ha="center",
                va="bottom",
                color="white",
